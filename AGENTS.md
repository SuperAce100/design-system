...
import { FancyToggle } from "@/registry/new-york/blocks/fancy-toggle/fancy-toggle";
...
Single Next.js 16 app — a UI component registry / design system. No databases, Docker, or external services required. The only optional external dependency is an `OPENAI_API_KEY` for the Chat demo's `/api/chat` route; everything else is fully client-side.

### Running the dev server

```
bun run dev          # next dev --turbopack on port 3000
```

### Available scripts (see `package.json`)

- `bun run lint` — ESLint (pre-existing warnings/errors exist in the repo)
- `bun run typecheck` — `tsc --noEmit`
- `bun run build` — production build
- `bun run registry:build` — rebuild `public/r/*.json` artifacts from `registry.json` (only needed when modifying the registry)

### Gotchas

- Bun v1.2.13 is the declared `packageManager`. The update script installs it automatically.
- `public/r/*.json` files are auto-generated by `bun run registry:build` (alias for `shadcn build`). Do not edit them by hand.
- The Chat component demo requires `OPENAI_API_KEY` env var to get AI responses; without it the message still renders in the UI but no reply comes back.
